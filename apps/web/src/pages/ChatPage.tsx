...
export default function ChatPage() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-[16rem_1fr] gap-4">
      <aside className="bg-white shadow rounded p-3">
        <button
        ...
        <ul className="mt-3 space-y-1 text-sm">
            ...
            <li key={c.id} className="flex items-center gap-1">
              <button
                ...
                className={`flex-1 text-left rounded px-2 py-1 truncate ${
                  conversationId === c.id ? "bg-brand-100 text-brand-700" : "hover:bg-slate-100"
                }`}
              >
                {c.title || "Untitled"}
              </button>
          ...
          {conversations.length === 0 && (
            <li className="text-xs text-slate-400 px-1">No conversations yet.</li>
          )}
        </ul>
      </aside>

      <section className="bg-white shadow rounded flex flex-col h-[70vh]">
        ...
        >
          <button
            ...
            className={`rounded px-2 py-1.5 text-sm border ${
              mic.recording ? "bg-rose-600 text-white border-rose-600" : "border-slate-300 hover:bg-slate-100"
            }`}
          ...
          <input
            ...
            className="flex-1 rounded border-slate-300 border px-3 py-1.5 text-sm"
          />
          <button
      ...
      >
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1">
          {message.detectedLanguage && message.detectedLanguage !== "en" && (
            <span className="text-xs rounded bg-white/70 text-brand-700 px-1.5 py-0.5">
          ...
          {!isUser && (
            <button
              ...
              className="text-xs rounded bg-white/70 text-brand-700 px-1.5 py-0.5 hover:underline"
              title="Read aloud"
            >
              🔊
            </button>
          ...
          {message.citations?.map((c) => (
            <a
              ...
              className="text-xs rounded bg-white/80 text-brand-700 px-1.5 py-0.5 hover:underline"
            >
              {c.case_id}
            </a>
          ))}
